    ...
    @foreach($tasks as $task)
        @if (strtotime($task->deadline) < strtotime('today'))
            <div style="background-color: red;">
                <h3>{{ $task->title }}</h3>
                <p>{{ $task->deadline }}</p>
                <!-- Здесь можно разместить специальный стиль или элемент для просроченных заданий -->
            </div>
        @endif
    @endforeach
</div>
    ...
    @foreach($tasks as $task)
        @if ($task->verified)
            <div style="background-color: green;">
                <h3>{{ $task->title }}</h3>
                <p>{{ $task->deadline }}</p>
                <!-- Здесь можно разместить специальный стиль или элемент для подтвержденных заданий -->
            </div>
        @endif
    @endforeach
</div>
